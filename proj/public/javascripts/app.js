 ...
 */
"use strict";
var app = angular.module('poking'
    , [ 'ui.bootstrap','ngRoute','ngResource','poking.RestfulSvc' ]);
...
app.run(function(){
    ...
    adminSvc.searchUniversity().$promise
        .then(function(res){
            if(res.success){
                let result = res.result;
                basicInfoSvc.universityCode = result;
                // basicInfoSvc.universityCode.unshift({code : null,value : null});
                console.log(basicInfoSvc)
            }
            else{
                alert("초기화 실패 : 유저검색 문제 있을수 있음");
                console.log(basicInfoSvc)
            }
        })
});

app.factory('basicInfoSvc',function(){
    var SVC = {};
    SVC.universityCode = [];
    SVC.majorCode = [];
    SVC.locationCode = [];
...
app.factory('userInfoSvc',function($rootScope,adminSvc){
    var user = {};

    $rootScope.getUserInfo = function(userid){
        adminSvc.getUserInfo(userid)
            .$promise
            .then(function(res){
                console.log(res);
                if(res.success){
                    console.log('success!!');
                    user.info = res.result.user;
                }
                else{
                    alert('server Err');
                }
    ...
    $rootScope.changeNick = function(id,nick){
        if(nick != null && nick != ""){
            console.log(nick);
            adminSvc.changeNick(id,nick).$promise
                .then(function(){
...
app.controller('userInfoCtrl',function($scope,userInfoSvc){
    $scope.user = userInfoSvc.info;
    $scope.$watch(function () {
        return userInfoSvc.info;
...
app.controller('navCtrl',function($scope,$window,Auth){
